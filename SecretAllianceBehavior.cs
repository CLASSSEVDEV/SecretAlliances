...
namespace SecretAlliances
{
    public class SecretAllianceBehavior : CampaignBehaviorBase
    {
        ...
        public override void RegisterEvents()
        {
            ...
            CampaignEvents.HeroMarried.AddNonSerializedListener(this, OnHeroMarried);
            CampaignEvents.OnSettlementOwnerChangedEvent.AddNonSerializedListener(this, OnSettlementOwnerChanged);
            CampaignEvents.CaravanTransactionCompleted.AddNonSerializedListener(this, OnCaravanTransaction);
            CampaignEvents.HeroPrisonerTaken.AddNonSerializedListener(this, OnHeroPrisonerTaken);
            CampaignEvents.HeroPrisonerReleased.AddNonSerializedListener(this, OnHeroPrisonerReleased);
            CampaignEvents.VillageBeingRaided.AddNonSerializedListener(this, OnVillageRaided);
            CampaignEvents.RaidCompleted.AddNonSerializedListener(this, OnRaidCompleted);
        }
    ...
    : new List<PartyBase>();
        ...
        private void OnHeroMarried(Hero hero, Hero spouse)
        {
            ...
            if (alliance != null)
            {
                // Marriage strengthens existing alliance
            ...
            {
                // Marriage might create new alliance
        ...
        private void OnSettlementOwnerChanged(Settlement settlement, Hero newOwner, Hero oldOwner, Hero capturerHero, ChangeOwnerOfSettlementAction.ChangeOwnerOfSettlementDetail detail)
        {
            if (newOwner?.Clan == null) return;

            // Check for territory agreements that might be affected
            var relevantAlliances = _alliances.Where(a => a.IsActive && 
            ...
            foreach (var alliance in relevantAlliances)
            {
                ...
                if (otherClan != null)
                {
                    // Territory changes affect alliance dynamics
        ...
        private void OnCaravanTransaction(MobileParty caravan, Town town, int gold)
        {
            if (caravan?.LeaderHero?.Clan == null || town?.Owner?.Clan == null) return;

            // Track trade transfers for economic network analysis
            var transferRecord = new TradeTransferRecord
            {
                FromClan = caravan.LeaderHero.Clan.Id,
                ToClan = town.Owner.Clan.Id,
            ...
            if (!_recentTransfers.ContainsKey(caravan.LeaderHero.Clan.Id))
            {
                _recentTransfers[caravan.LeaderHero.Clan.Id] = new List<TradeTransferRecord>();
            }
            _recentTransfers[caravan.LeaderHero.Clan.Id].Add(transferRecord);

            // Check for trade pact benefits
            var alliance = FindAlliance(caravan.LeaderHero.Clan, town.Owner.Clan);
        ...
        private void OnHeroPrisonerReleased(Hero prisoner, PartyBase party, IFaction capturerFaction, EndCaptivityDetail detail)
        {
            ...
            if (alliance != null && detail == EndCaptivityDetail.ReleasedWithoutRansom)
            {
        ...
        private void OnVillageRaided(Village village, MobileParty raiderParty)
        {
            if (village?.Owner?.Clan == null || raiderParty?.LeaderHero?.Clan == null) return;

            // Check for alliance implications
            var alliance = FindAlliance(raiderParty.LeaderHero.Clan, village.Owner.Clan);
            if (alliance != null)
            {
                // Raiding ally damages alliance severely
                alliance.TrustLevel -= 0.15f;
                alliance.Strength -= 0.1f;
        ...
        private void OnRaidCompleted(BattleSideEnum winnerSide, RaidEventComponent raidEvent)
        {
            ...
            {
                // Successful raids against enemies strengthen alliances with similar enemies
                var alliances = _alliances.Where(a => a.IsActive &&
        ...
        public void ExecuteEconomicWarfare(SecretAllianceRecord alliance, Clan targetClan)
        {
            ...
            if (targetClan.Gold > 1000)
            {
                ...
                foreach (var party in targetClan.WarParties)
                {
                    if (party.MobileParty?.IsCaravan == true)
                    {
                        party.MobileParty.Ai.SetDoNotMakeNewDecisions(true);
        ...
        public void InitiateJointCampaign(SecretAllianceRecord alliance, Settlement targetSettlement)
        {
            ...
            if (initiator?.WarParties.Any() != true || target?.WarParties.Any() != true) return;

            // Coordinate military parties
            var initiatorParty = initiator.WarParties.FirstOrDefault()?.MobileParty;
            var targetParty = target.WarParties.FirstOrDefault()?.MobileParty;
