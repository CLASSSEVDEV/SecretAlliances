...
namespace SecretAlliances
{
    ...
    : new List<PartyBase>();
        ...
        // === MEGA FEATURE EXPANSION METHODS ===

        private void PerformIntegrityRepair()
        {
            if (_config?.DebugVerbose == true)
            {
                Debug.Print("[SecretAlliances] Performing integrity repair...");
            }

            // Remove duplicate (Initiator,Target) combos keeping highest Strength
            var duplicateGroups = _alliances
                .Where(a => a.IsActive)
                .GroupBy(a => new { InitiatorId = a.InitiatorClanId, TargetId = a.TargetClanId })
                .Where(g => g.Count() > 1);

            foreach (var group in duplicateGroups)
            {
                var toKeep = group.OrderByDescending(a => a.Strength).First();
                var toRemove = group.Where(a => a != toKeep);
                ...
                foreach (var alliance in toRemove)
                {
                    ...
                    if (_config?.DebugVerbose == true)
                    {
                        Debug.Print($"[SecretAlliances] Removed duplicate alliance {alliance.InitiatorClanId}-{alliance.TargetClanId}");
                    }
                }
            }

            // Reassign missing UniqueId
            foreach (var alliance in _alliances.Where(a => a.IsActive))
            {
                if (alliance.UniqueId == null || alliance.UniqueId == MBGUID.Empty)
                {
                    var initiator = alliance.GetInitiatorClan();
                    var target = alliance.GetTargetClan();
                    if (initiator != null && target != null)
                    {
                        alliance.UniqueId = MBGUID.Parse($"{initiator.StringId}_{target.StringId}");
                        if (_config?.DebugVerbose == true)
                        {
                            Debug.Print($"[SecretAlliances] Assigned UniqueId to alliance: {alliance.UniqueId}");
                        }
                    }
                }
            }
        ...
        private void PrintSelfDiagnostics()
        {
            int activeCount = _alliances.Count(a => a.IsActive);
            float avgStrength = activeCount > 0 ? _alliances.Where(a => a.IsActive).Average(a => a.Strength) : 0f;
            float avgSecrecy = activeCount > 0 ? _alliances.Where(a => a.IsActive).Average(a => a.Secrecy) : 0f;
            int intelCount = _intelligence.Count;

            Debug.Print($"[SecretAlliances] Diagnostics - Active Alliances: {activeCount}, Avg Strength: {avgStrength:F2}, Avg Secrecy: {avgSecrecy:F2}, Intel Records: {intelCount}");
        }

        private void EvaluateAllianceEnhanced(SecretAllianceRecord alliance, int currentDay)
        {
            // Call existing evaluation method
            EvaluateAlliance(alliance);

            // Enhanced leak processing with config-based probabilities
            ProcessLeakLogic(alliance, currentDay);

            // Process betrayal cooldown
            if (alliance.BetrayalCooldownDay > 0 && currentDay >= alliance.BetrayalCooldownDay)
            {
                alliance.BetrayalCooldownDay = 0;
                if (_config?.DebugVerbose == true)
                {
                    Debug.Print($"[SecretAlliances] Betrayal cooldown expired for alliance {alliance.UniqueId}");
                }
            }

            // Process counter-intel buff expiry
            if (alliance.CounterIntelBuffExpiryDay > 0 && currentDay >= alliance.CounterIntelBuffExpiryDay)
            {
                alliance.CounterIntelBuffExpiryDay = 0;
                if (_config?.DebugVerbose == true)
                {
                    Debug.Print($"[SecretAlliances] Counter-intelligence buff expired for alliance {alliance.UniqueId}");
                }
            }
        }

        private void ProcessOperationsScheduler(SecretAllianceRecord alliance, int currentDay)
        {
            // Check if eligible for operations
            if (currentDay >= alliance.NextEligibleOperationDay && alliance.PendingOperationType == 0)
            {
                ScheduleOperation(alliance, currentDay);
            }

            // Execute pending operations
            if (alliance.PendingOperationType > 0)
            {
                ExecuteOperation(alliance, currentDay);
            }
        }

        private void ScheduleOperation(SecretAllianceRecord alliance, int currentDay)
        {
            var availableOps = new List<(OperationType op, float weight)>();

            // Check which operations are off cooldown and add with weights
            if (!alliance.IsOperationOnCooldown(OperationType.CovertAid, currentDay))
                availableOps.Add((OperationType.CovertAid, 1.0f));
            
            if (!alliance.IsOperationOnCooldown(OperationType.SpyProbe, currentDay))
                availableOps.Add((OperationType.SpyProbe, 1.0f));
            
            if (!alliance.IsOperationOnCooldown(OperationType.RecruitmentFeelers, currentDay))
                availableOps.Add((OperationType.RecruitmentFeelers, 0.8f));
            
            if (!alliance.IsOperationOnCooldown(OperationType.SabotageRaid, currentDay))
                availableOps.Add((OperationType.SabotageRaid, 0.6f));
            
            if (!alliance.IsOperationOnCooldown(OperationType.CounterIntelligence, currentDay))
                availableOps.Add((OperationType.CounterIntelligence, 0.7f));

            if (availableOps.Count == 0) return;

            // Adjust weights based on alliance state
            for (int i = 0; i < availableOps.Count; i++)
            {
                var (op, weight) = availableOps[i];
                
                if (alliance.PoliticalPressure > 0.6f)
                    weight *= 1.2f; // Higher pressure increases activity
                
                if (alliance.TrustLevel > 0.75f)
                    weight *= 1.1f; // High trust enables more operations
                
                availableOps[i] = (op, weight);
            }

            // Weighted random selection
            float totalWeight = availableOps.Sum(op => op.weight);
            float random = MBRandom.RandomFloat * totalWeight;
            
            OperationType selectedOp = OperationType.CovertAid;
            float currentWeight = 0f;
            
            foreach (var (op, weight) in availableOps)
            {
            ...
            // Set operation
            alliance.PendingOperationType = (int)selectedOp;
            alliance.LastOperationDay = currentDay;

            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Scheduled {selectedOp} for alliance {alliance.UniqueId}");
            }
        }

        private void ExecuteOperation(SecretAllianceRecord alliance, int currentDay)
        {
            var operationType = (OperationType)alliance.PendingOperationType;
            
            switch (operationType)
            {
                case OperationType.CovertAid:
                    ExecuteCovertAid(alliance, currentDay);
                    break;
                case OperationType.SpyProbe:
                    ExecuteSpyProbe(alliance, currentDay);
                    break;
                case OperationType.RecruitmentFeelers:
                    ExecuteRecruitmentFeelers(alliance, currentDay);
                    break;
                case OperationType.SabotageRaid:
                    ExecuteSabotageRaid(alliance, currentDay);
                    break;
                case OperationType.CounterIntelligence:
                    ExecuteCounterIntelligence(alliance, currentDay);
                    break;
            }

            // Set cooldown and next eligible day
            int cooldownDays = GetOperationCooldown(operationType);
            alliance.SetCooldownEnd(operationType, currentDay + cooldownDays);
            
            int intervalDays = _config?.OperationIntervalDays ?? 7;
            int adaptiveMinDays = _config?.OperationAdaptiveMinDays ?? 3;
            
            // Reduce interval if high pressure/trust but never below min
            if (alliance.PoliticalPressure > 0.6f || alliance.TrustLevel > 0.75f)
            {
                intervalDays = Math.Max(adaptiveMinDays, intervalDays - 2);
            }
            
            alliance.NextEligibleOperationDay = currentDay + intervalDays;
            alliance.PendingOperationType = 0;

            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Executed {operationType} for alliance {alliance.UniqueId}, next eligible: {alliance.NextEligibleOperationDay}");
            }
        }
        ...
        private int GetOperationCooldown(OperationType operationType)
        {
            ...
            {
                OperationType.CovertAid => _config?.CovertAidCooldownDays ?? 4,
                OperationType.SpyProbe => _config?.SpyProbeCooldownDays ?? 5,
                OperationType.RecruitmentFeelers => _config?.RecruitmentCooldownDays ?? 14,
                OperationType.SabotageRaid => _config?.SabotageCooldownDays ?? 10,
                OperationType.CounterIntelligence => _config?.CounterIntelCooldownDays ?? 8,
                _ => 7
            };
        }

        private void ProcessLeakLogic(SecretAllianceRecord alliance, int currentDay)
        {
            // Calculate logistic leak probability
            float baseChance = _config?.LeakBaseChance ?? 0.01f;
            float logisticMultiplier = 1f + (alliance.LeakAttempts * 0.1f); // Each attempt increases future risk
            
            // Apply counter-intel buff if active
            if (alliance.CounterIntelBuffExpiryDay > currentDay)
            {
                logisticMultiplier *= 0.8f; // Reduce leak chance while buff active
            }
            
            float finalProbability = Math.Min(0.8f, baseChance * logisticMultiplier);
            
            if (MBRandom.RandomFloat < finalProbability)
            {
                alliance.LeakAttempts++;
                ProcessLeak(alliance, currentDay);
            }
        }

        private void ProcessLeak(SecretAllianceRecord alliance, int currentDay)
        {
            var initiator = alliance.GetInitiatorClan();
            var target = alliance.GetTargetClan();
            ...
            var potentialInformants = GetPotentialInformants(initiator, target, alliance);
            if (!potentialInformants.Any()) return;

            var informant = potentialInformants[MBRandom.RandomInt(potentialInformants.Count)];
            
            // Choose intel category based on last operation
            AllianceIntelType category = AllianceIntelType.GeneralRumor;
            if (alliance.LastOperationDay == currentDay - 1)
            {
                var lastOp = (OperationType)alliance.PendingOperationType;
                category = lastOp switch
                {
                    OperationType.CovertAid => AllianceIntelType.TradePactEvidence,
                    OperationType.SabotageRaid => AllianceIntelType.MilitaryCoordination,
                    OperationType.SpyProbe => AllianceIntelType.MilitaryCoordination,
                    _ => AllianceIntelType.GeneralRumor
                };
            }

            // Calculate reliability and severity
            float baseReliability = 0.5f;
            float secrecyModifier = (1f - alliance.Secrecy) * 0.3f;
            float randomModifier = MBRandom.RandomFloat * 0.5f + 0.75f; // 0.75-1.25 range
            float reliability = Math.Max(0.05f, baseReliability + secrecyModifier) * randomModifier;

            float severity = 0.4f * (1f - alliance.Secrecy) + 0.3f * alliance.Strength + 0.3f * MBRandom.RandomFloat;

            var intel = new AllianceIntelligence
            {
                AllianceId = alliance.UniqueId,
                InformerHeroId = informant.Id,
                ReliabilityScore = Math.Min(1f, reliability),
                DaysOld = 0,
                IsConfirmed = false,
                SeverityLevel = Math.Min(1f, severity),
                ClanAId = alliance.InitiatorClanId,
                ClanBId = alliance.TargetClanId,
            ...
            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Leak detected - {category} intel by {informant.Name}, severity: {severity:F2}");
            }
        }

        private void ProcessFormationAI(int currentDay)
        {
            if (_config == null) return;

            int maxFormations = _config.MaxDailyFormations;
            int formationsToday = 0;

            // Collect candidate clan pairs
            var candidates = new List<(Clan initiator, Clan target, float score)>();

            foreach (var initiator in Clan.All.Where(c => c.IsActive && !c.IsEliminated))
            {
                foreach (var target in Clan.All.Where(c => c.IsActive && !c.IsEliminated && c != initiator))
                {
                    if (HasExistingAlliance(initiator, target)) continue;
                    if (initiator.Kingdom == target.Kingdom) continue; // Don't ally with same kingdom (or adjust based on requirements)

                    float score = CalculateFormationScore(initiator, target);
                    if (score > 0.1f) // Minimum threshold
                    {
                        candidates.Add((initiator, target, score));
                    }
                }
            }

            // Sort by score and attempt formations
            candidates = candidates.OrderByDescending(c => c.score).ToList();

            foreach (var (initiator, target, score) in candidates.Take(maxFormations))
            {
                if (formationsToday >= maxFormations) break;

                float formationChance = _config.FormationBaseChance * Math.Min(2f, score); // Cap multiplier
                
                if (MBRandom.RandomFloat < formationChance)
                {
                    CreateNewAlliance(initiator, target, score);
                    formationsToday++;
                    
                    if (_config.DebugVerbose)
                    {
                        Debug.Print($"[SecretAlliances] New alliance formed: {initiator.Name} - {target.Name} (score: {score:F2})");
                    }
                }
            }
        }

        private float CalculateFormationScore(Clan initiator, Clan target)
        {
            float score = 0f;
            ...
            // Economic disparity (moderate disparity can be beneficial)
            float economicRatio = Math.Min(initiator.Gold, target.Gold) / (float)Math.Max(1, Math.Max(initiator.Gold, target.Gold));
            score += economicRatio * 0.2f;

            // Desperation factor (weak clans more likely to ally)
            float initiatorStrength = GetClanStrength(initiator);
            float targetStrength = GetClanStrength(target);
            
            if (initiatorStrength < 0.3f || targetStrength < 0.3f)
                score += 0.25f;

            // Distance penalty (closer clans more likely to ally)
            float distance = GetClanDistance(initiator, target);
            if (distance > 0)
                score *= Math.Max(0.5f, 1f - (distance / 200f)); // Reduce score for distant clans

            return Math.Max(0f, score);
        }

        private float GetClanStrength(Clan clan)
        {
            if (clan?.Kingdom == null) return 0.1f;
            
            float relativeStrength = (float)clan.TotalStrength / Math.Max(1f, clan.Kingdom.TotalStrength);
            return Math.Min(1f, relativeStrength * 10f); // Scale appropriately
        }

        private void CreateNewAlliance(Clan initiator, Clan target, float baseScore)
        {
            var alliance = new SecretAllianceRecord
            {
                InitiatorClanId = initiator.Id,
                TargetClanId = target.Id,
                UniqueId = MBGUID.Parse($"{initiator.StringId}_{target.StringId}"),
                Strength = 0.1f + (baseScore * 0.3f), // Start with low strength
                Secrecy = 0.8f + (MBRandom.RandomFloat * 0.15f), // Start with high secrecy
                TrustLevel = 0.2f + (baseScore * 0.2f),
                IsActive = true,
                CreatedGameDay = CampaignTime.Now.GetDayOfYear,
                GroupId = 0, // Will be assigned if part of coalition later
                NextEligibleOperationDay = CampaignTime.Now.GetDayOfYear + (_config?.OperationIntervalDays ?? 7)
            };

            // Set initial political pressure and incentives
            alliance.PoliticalPressure = CalculatePoliticalPressure(initiator, target);
            alliance.EconomicIncentive = CalculateEconomicIncentive(initiator, target);
            alliance.HasCommonEnemies = HasCommonEnemies(initiator, target);
            ...
            {
                SendAllianceNotification($"Secret alliance formed between {initiator.Name} and {target.Name}", "Alliance Formation");
            }
        }

        private float CalculateEconomicIncentive(Clan initiator, Clan target)
        {
            float tradeBenefit = 0f;
            
            // Simple trade benefit calculation based on settlement ownership
            if (initiator.Settlements.Any() && target.Settlements.Any())
            {
                tradeBenefit = 0.3f + (MBRandom.RandomFloat * 0.4f);
            }
        ...
        private void ProcessIntelligenceAging()
        {
            ...
            {
                intel.DaysOld++;
                intel.ReliabilityScore = Math.Max(0.05f, intel.ReliabilityScore - 0.005f);

                // Remove very old or unreliable intel
                if (intel.DaysOld > 60 || intel.ReliabilityScore < 0.1f)
                {
                    toRemove.Add(intel);
                }
        ...
        private void ProcessForcedReveals(int currentDay)
        {
            if (_config == null) return;

            foreach (var alliance in _alliances.Where(a => a.IsActive && !a.BetrayalRevealed))
            {
                if (alliance.Strength > _config.ForcedRevealStrengthThreshold && 
                    alliance.Secrecy < _config.ForcedRevealSecrecyThreshold)
                {
                    float revealProb = (alliance.Strength - _config.ForcedRevealStrengthThreshold) * 
                                     (_config.ForcedRevealSecrecyThreshold - alliance.Secrecy + 0.05f);

                    if (MBRandom.RandomFloat < revealProb)
                    {
                        ProcessForcedReveal(alliance, currentDay);
                    }
                }
            }
        }

        private void ProcessForcedReveal(SecretAllianceRecord alliance, int currentDay)
        {
            alliance.BetrayalRevealed = true;
            alliance.Secrecy = Math.Max(0f, alliance.Secrecy - 0.4f);

            // Create intel for all same-kingdom clans
            var initiator = alliance.GetInitiatorClan();
            var target = alliance.GetTargetClan();
            
            if (initiator?.Kingdom != null)
            {
                CreateGeneralIntelForKingdom(alliance, initiator.Kingdom);
            }
            if (target?.Kingdom != null && target.Kingdom != initiator?.Kingdom)
            {
                CreateGeneralIntelForKingdom(alliance, target.Kingdom);
            }

            if (_config?.NotificationsEnabled == true)
            {
                SendAllianceNotification($"Alliance between {initiator?.Name} and {target?.Name} has been exposed!", "Alliance Revealed");
            }

            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Forced reveal for alliance {alliance.UniqueId}");
            }
        }

        private void CreateGeneralIntelForKingdom(SecretAllianceRecord alliance, Kingdom kingdom)
        {
            foreach (var clan in kingdom.Clans.Where(c => c.IsActive))
            {
                ...
                if (hero != null)
                {
                    ...
                    {
                        AllianceId = alliance.UniqueId,
                        InformerHeroId = hero.Id,
                        ReliabilityScore = 0.9f, // Forced reveals are highly reliable
                        DaysOld = 0,
                        IsConfirmed = true,
                        SeverityLevel = 0.8f,
                        ClanAId = alliance.InitiatorClanId,
        ...
        private void ProcessAgingAndDissolution(SecretAllianceRecord alliance, int currentDay)
        {
            int ageDays = currentDay - alliance.CreatedGameDay;
            
            if (ageDays > 540) // About 1.5 years
            {
                // Check for dissolution conditions
                if (alliance.Strength < 0.2f || 
                    (alliance.Strength < 0.4f && alliance.TrustLevel < 0.3f))
                {
                    float dissolutionChance = 0.05f + Math.Max(0f, 0.3f - alliance.Strength);
                    
                    if (MBRandom.RandomFloat < dissolutionChance)
                    {
                        alliance.IsActive = false;
                        ...
                        if (_config?.NotificationsEnabled == true)
                        {
                            var initiator = alliance.GetInitiatorClan();
                            var target = alliance.GetTargetClan();
                            SendAllianceNotification($"Alliance between {initiator?.Name} and {target?.Name} has dissolved due to neglect.", "Alliance Dissolved");
                        }

                        if (_config?.DebugVerbose == true)
                        {
                            Debug.Print($"[SecretAlliances] Alliance {alliance.UniqueId} dissolved due to aging");
                        }
                    }
                }
            }
        ...
        private void SendAllianceNotification(string message, string title)
        {
        ...
        private void ExecuteCovertAid(SecretAllianceRecord alliance, int currentDay)
        {
            float strengthGain = 0.01f + (MBRandom.RandomFloat * 0.02f); // 0.01-0.03 range
            float trustGain = 0.01f + (MBRandom.RandomFloat * 0.01f); // 0.01-0.02 range
            float secrecyLoss = 0.005f + (MBRandom.RandomFloat * 0.01f); // 0.005-0.015 range

            alliance.Strength = Math.Min(1f, alliance.Strength + strengthGain);
            alliance.TrustLevel = Math.Min(1f, alliance.TrustLevel + trustGain);
            alliance.Secrecy = Math.Max(0f, alliance.Secrecy - secrecyLoss);

            // Possible intel generation if significant strength gain
            if (strengthGain > 0.02f && MBRandom.RandomFloat < 0.4f)
            {
                CreateFinancialIntel(alliance, "moderate");
            }
        }

        private void ExecuteSpyProbe(SecretAllianceRecord alliance, int currentDay)
        {
            float difficulty = 0.5f + MBRandom.RandomFloat * 0.3f;
            bool success = MBRandom.RandomFloat > difficulty;

            if (success)
            {
                // Create intel about enemy clan
                CreateEnemyIntel(alliance, MBRandom.RandomFloat < 0.5f ? "Military" : "General");
            }
            else
            {
                // Failure consequences
                alliance.Secrecy = Math.Max(0f, alliance.Secrecy - 0.05f);
                
                // 40% of logistic final leak probability
                float leakChance = (_config?.LeakBaseChance ?? 0.01f) * (1f + alliance.LeakAttempts * 0.1f) * 0.4f;
                if (MBRandom.RandomFloat < leakChance)
                {
                    ProcessLeak(alliance, currentDay);
                }
        ...
        private void ExecuteRecruitmentFeelers(SecretAllianceRecord alliance, int currentDay)
        {
            // Find candidate clan
            var candidateClan = FindRecruitmentCandidate(alliance);
            if (candidateClan == null) return;

            float chance = Math.Min(0.8f, alliance.Strength * 0.4f + alliance.TrustLevel * 0.3f + 0.3f);
            
            if (MBRandom.RandomFloat < chance)
            {
                // Success - create new alliance with inherited GroupId
                CreateRecruitedAlliance(alliance, candidateClan);
            }
            else
            {
                // Failure - small secrecy loss
                alliance.Secrecy = Math.Max(0f, alliance.Secrecy - 0.02f);
            }
        }

        private void ExecuteSabotageRaid(SecretAllianceRecord alliance, int currentDay)
        {
            // Pick rival clan (simplified)
            var rivals = GetRivalClans(alliance);
            if (!rivals.Any()) return;

            var target = rivals[MBRandom.RandomInt(rivals.Count)];
            
            // Log virtual effect (TODO: integrate with actual game systems)
            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Sabotage raid executed against {target.Name} (virtual effect)");
            }

            // Generate high severity military intel
            CreateMilitaryIntel(alliance, "high");

            // Risk of leak if high severity
            float severity = 0.6f + (MBRandom.RandomFloat * 0.4f);
            if (severity > 0.5f)
            {
                float leakChance = (_config?.LeakBaseChance ?? 0.01f) * 2f; // Doubled risk
                if (MBRandom.RandomFloat < leakChance)
                {
                    ProcessLeak(alliance, currentDay);
                }
        ...
        private void ExecuteCounterIntelligence(SecretAllianceRecord alliance, int currentDay)
        {
            // Apply buff
            alliance.CounterIntelBuffExpiryDay = currentDay + 14; // 2 week buff

            // Decrease existing intel reliability
            var relevantIntel = _intelligence.Where(i => 
                i.ClanAId == alliance.InitiatorClanId || i.ClanAId == alliance.TargetClanId ||
                i.ClanBId == alliance.InitiatorClanId || i.ClanBId == alliance.TargetClanId).ToList();

            foreach (var intel in relevantIntel)
            {
                float reduction = 0.1f + (MBRandom.RandomFloat * 0.2f); // 10-30% reduction
                intel.ReliabilityScore = Math.Max(0.05f, intel.ReliabilityScore - reduction);
            }

            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Counter-intelligence executed, buff active until day {alliance.CounterIntelBuffExpiryDay}");
            }
        }

        // === RUMOR API METHODS ===

        public string GetTopRumorString(Hero hero, Clan playerClan, AllianceIntelType category = AllianceIntelType.GeneralRumor)
        {
            var rumors = GetRumorList(hero, playerClan, 1);
            return rumors.FirstOrDefault() ?? "No rumors available.";
        }

        public List<string> GetRumorList(Hero hero, Clan playerClan, int maxCount)
        {
            if (_config == null)
                maxCount = Math.Min(maxCount, 3); // Default fallback
            else
                maxCount = Math.Min(maxCount, _config.MaxRumorsReturned);

            var rumors = new List<string>();
            
            // Defensive checks
            if (hero == null || playerClan == null) return rumors;

            var relevantIntel = _intelligence
                .Where(i => i.ReliabilityScore > 0.2f && i.DaysOld < 30)
                .OrderByDescending(i => i.SeverityLevel * i.ReliabilityScore)
                .Take(maxCount)
                .ToList();

            foreach (var intel in relevantIntel)
            {
                string rumorText = GenerateRumorText(intel, hero, playerClan);
                if (!string.IsNullOrEmpty(rumorText))
                {
                    rumors.Add(rumorText);
                }
            }
        ...
        private string GenerateRumorText(AllianceIntelligence intel, Hero hero, Clan playerClan)
        {
            ...
            if (clanA == null || clanB == null) return null;

            var category = (AllianceIntelType)intel.IntelCategory;
            
            return category switch
            {
                AllianceIntelType.GeneralRumor => $"There are whispers of secret dealings between {clanA.Name} and {clanB.Name}.",
                AllianceIntelType.TradePactEvidence => $"Merchants report unusual trade flows between {clanA.Name} and {clanB.Name}.",
                AllianceIntelType.MilitaryCoordination => $"Scouts have noticed coordinated movements between {clanA.Name} and {clanB.Name} forces.",
                AllianceIntelType.SecretMeeting => $"A secret meeting between {clanA.Name} and {clanB.Name} representatives was reported.",
                AllianceIntelType.BetrayalPlot => $"There are dark rumors of betrayal involving {clanA.Name} and {clanB.Name}.",
                _ => $"Strange activities have been reported involving {clanA.Name} and {clanB.Name}."
            };
        }

        // === HELPER METHODS ===

        private void CreateFinancialIntel(SecretAllianceRecord alliance, string severity)
        {
            var initiator = alliance.GetInitiatorClan();
            var target = alliance.GetTargetClan();
            if (initiator == null || target == null) return;
            ...
            float severityLevel = severity switch
            {
                "low" => 0.2f + (MBRandom.RandomFloat * 0.2f),
                "moderate" => 0.4f + (MBRandom.RandomFloat * 0.2f),
                "high" => 0.7f + (MBRandom.RandomFloat * 0.3f),
                _ => 0.4f
            };
            ...
            var intel = new AllianceIntelligence
            {
                AllianceId = alliance.UniqueId,
                InformerHeroId = informant.Id,
                ReliabilityScore = CalculateInformerReliability(informant) * 0.9f,
                DaysOld = 0,
                IsConfirmed = false,
                SeverityLevel = Math.Min(1f, severityLevel),
                ClanAId = alliance.InitiatorClanId,
                ClanBId = alliance.TargetClanId,
                IntelCategory = (int)AllianceIntelType.TradePactEvidence
            };
        ...
        private void CreateEnemyIntel(SecretAllianceRecord alliance, string type)
        {
            var initiator = alliance.GetInitiatorClan();
            var target = alliance.GetTargetClan();
            if (initiator == null || target == null) return;

            // Find a random enemy clan
            var enemyClans = GetRivalClans(alliance);
            if (!enemyClans.Any()) return;

            var enemyClan = enemyClans[MBRandom.RandomInt(enemyClans.Count)];
            var enemyHero = enemyClan.Heroes?.FirstOrDefault();
            if (enemyHero == null) return;

            AllianceIntelType category = type switch
            {
                "Military" => AllianceIntelType.MilitaryCoordination,
                "General" => AllianceIntelType.GeneralRumor,
                _ => AllianceIntelType.GeneralRumor
            };
            ...
            {
                AllianceId = alliance.UniqueId,
                InformerHeroId = enemyHero.Id,
                ReliabilityScore = 0.6f + (MBRandom.RandomFloat * 0.3f),
                DaysOld = 0,
                IsConfirmed = false,
                SeverityLevel = 0.5f + (MBRandom.RandomFloat * 0.3f),
                ClanAId = alliance.InitiatorClanId,
                ClanBId = alliance.TargetClanId,
                IntelCategory = (int)category
            };
        ...
        private void CreateMilitaryIntel(SecretAllianceRecord alliance, string severity)
        {
            var initiator = alliance.GetInitiatorClan();
            var target = alliance.GetTargetClan();
            if (initiator == null || target == null) return;
            ...
            float severityLevel = severity switch
            {
                "low" => 0.3f + (MBRandom.RandomFloat * 0.2f),
                "moderate" => 0.5f + (MBRandom.RandomFloat * 0.2f),
                "high" => 0.8f + (MBRandom.RandomFloat * 0.2f),
                _ => 0.6f
            };
        ...
        private Clan FindRecruitmentCandidate(SecretAllianceRecord alliance)
        {
            ...
            // Find clans not already allied to either side
            return Clan.All
                .Where(c => c.IsActive && !c.IsEliminated && c != initiator && c != target)
                .Where(c => !HasExistingAlliance(c, initiator) && !HasExistingAlliance(c, target))
                .OrderByDescending(c => CalculateRecruitmentScore(c, alliance))
                .FirstOrDefault();
        }

        private float CalculateRecruitmentScore(Clan candidate, SecretAllianceRecord alliance)
        {
            // Simple scoring based on power complement and shared enemies
            float score = 0.3f; // Base score
            
            if (HasCommonEnemies(candidate, alliance.GetInitiatorClan()))
                score += 0.4f;
            
            if (HasCommonEnemies(candidate, alliance.GetTargetClan()))
                score += 0.3f;
        ...
        private void CreateRecruitedAlliance(SecretAllianceRecord parentAlliance, Clan newClan)
        {
            ...
            var newAlliance = new SecretAllianceRecord
            {
                ...
                UniqueId = MBGUID.Parse($"{initiator.StringId}_{newClan.StringId}"),
                GroupId = parentAlliance.GroupId > 0 ? parentAlliance.GroupId : _nextGroupId++,
                Strength = 0.15f,
                Secrecy = 0.7f,
                TrustLevel = 0.25f,
                IsActive = true,
            ...
            // Update parent alliance GroupId if it wasn't set
            if (parentAlliance.GroupId == 0)
            {
                parentAlliance.GroupId = newAlliance.GroupId;
            }

            if (_config?.DebugVerbose == true)
            {
                Debug.Print($"[SecretAlliances] Recruited {newClan.Name} into coalition {newAlliance.GroupId}");
            }
        }

        private List<Clan> GetRivalClans(SecretAllianceRecord alliance)
        {
            var initiator = alliance.GetInitiatorClan();
            var target = alliance.GetTargetClan();
            
            if (initiator?.Kingdom == null || target?.Kingdom == null) return new List<Clan>();

            // Return clans from kingdoms at war with either alliance member
            return Clan.All
                .Where(c => c.IsActive && !c.IsEliminated)
                .Where(c => (c.Kingdom != null && initiator.Kingdom.IsAtWarWith(c.Kingdom)) ||
                           (c.Kingdom != null && target.Kingdom.IsAtWarWith(c.Kingdom)))
                .ToList();
        }

        // === BETRAYAL SYSTEM (for battle events) ===

        private void OnBattleStarted(MapEvent mapEvent)
        {
            if (mapEvent == null) return;

            // Check for allied clans on opposing sides
            var side1Clans = mapEvent.AttackerSide.Parties.Select(p => p.LeaderHero?.Clan).Where(c => c != null).Distinct();
            var side2Clans = mapEvent.DefenderSide.Parties.Select(p => p.LeaderHero?.Clan).Where(c => c != null).Distinct();

            foreach (var clan1 in side1Clans)
            {
                foreach (var clan2 in side2Clans)
                {
                    var alliance = _alliances.FirstOrDefault(a => 
                        a.IsActive && 
                        ((a.InitiatorClanId == clan1.Id && a.TargetClanId == clan2.Id) ||
                         (a.InitiatorClanId == clan2.Id && a.TargetClanId == clan1.Id)));
                    ...
                    if (alliance != null)
                    {
                        EvaluateBetrayalProbability(alliance, CampaignTime.Now.GetDayOfYear);
                    }
                }
            }
        }

        private void EvaluateBetrayalProbability(SecretAllianceRecord alliance, int currentDay)
        {
            if (alliance.BetrayalCooldownDay > currentDay) return; // Still on cooldown

            float baseBetrayalChance = _config?.BetrayalBaseChance ?? 0.005f;
            float probability = baseBetrayalChance + 
                              (alliance.Strength * 0.2f) + 
                              ((1f - alliance.Secrecy) * 0.25f) + 
                              (alliance.PoliticalPressure * 0.15f) + 
                              (alliance.EscalationCounter * 0.05f);

            probability = Math.Min(0.8f, probability); // Cap at 80%

            if (_config?.DebugVerbose == true || probability > 0.05f)
            {
                var factors = new
                {
                    Base = baseBetrayalChance,
                    Strength = alliance.Strength * 0.2f,
                    Secrecy = (1f - alliance.Secrecy) * 0.25f,
                    Pressure = alliance.PoliticalPressure * 0.15f,
                    Escalation = alliance.EscalationCounter * 0.05f,
                    Total = probability
                };
                Debug.Print($"[SecretAlliances] Betrayal factors for {alliance.UniqueId}: {Newtonsoft.Json.JsonConvert.SerializeObject(factors)}");
            }

            if (MBRandom.RandomFloat < probability)
            {
                // Betrayal occurs
                ProcessBetrayal(alliance, currentDay);
            }
            else if (MBRandom.RandomFloat < probability * 1.25f)
            {
                // Near miss - escalate
                alliance.EscalationCounter = Math.Min(3, alliance.EscalationCounter + 1);
                if (_config?.DebugVerbose == true)
                {
                    Debug.Print($"[SecretAlliances] Betrayal escalation for {alliance.UniqueId}, counter: {alliance.EscalationCounter}");
                }
            }
        }

        private void ProcessBetrayal(SecretAllianceRecord alliance, int currentDay)
        {
            alliance.BetrayalRevealed = true;
            alliance.Strength = Math.Max(0f, alliance.Strength - 0.25f);
            alliance.TrustLevel = Math.Max(0f, alliance.TrustLevel - 0.4f);
            alliance.Secrecy = Math.Max(0f, alliance.Secrecy - 0.3f);
            alliance.BetrayalCooldownDay = currentDay + 30;
            alliance.EscalationCounter = 0; // Reset escalation
            ...
            if (_config?.NotificationsEnabled == true)
            {
                SendAllianceNotification($"BETRAYAL! The alliance between {initiator?.Name} and {target?.Name} has been broken in battle!", "Alliance Betrayal");
            }

            Debug.Print($"[SecretAlliances] Betrayal occurred for alliance {alliance.UniqueId}");
        }
    }
}