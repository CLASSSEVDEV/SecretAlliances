...
namespace SecretAlliances
{
    public static class ConsoleCommands
    {
        ...
        {
            // Register console commands for debugging and testing
            CommandLineArgumentManager.RegisterCommand("sa.dumpAlliances", "Dump all active secret alliances", DumpAlliances);
            CommandLineArgumentManager.RegisterCommand("sa.forceLeak", "Force a leak for alliance between two clans (usage: sa.forceLeak clanA clanB)", ForceLeak);
            CommandLineArgumentManager.RegisterCommand("sa.addTrust", "Add trust to alliance (usage: sa.addTrust clanA clanB amount)", AddTrust);
            CommandLineArgumentManager.RegisterCommand("sa.runOperation", "Run operation for alliance (usage: sa.runOperation clanA clanB opType)", RunOperation);
            CommandLineArgumentManager.RegisterCommand("sa.listIntel", "List intelligence for clan (usage: sa.listIntel [clanStringId])", ListIntel);
            CommandLineArgumentManager.RegisterCommand("sa.config", "Show or modify configuration (usage: sa.config [property] [value])", ConfigCommand);
            CommandLineArgumentManager.RegisterCommand("sa.forceReveal", "Force reveal an alliance (usage: sa.forceReveal clanA clanB)", ForceReveal);
            CommandLineArgumentManager.RegisterCommand("sa.createAlliance", "Create test alliance (usage: sa.createAlliance clanA clanB)", CreateAlliance);
        }
