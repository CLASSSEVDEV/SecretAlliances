...
namespace SecretAlliances.Behaviors
{
    ...
    public class LeakBehavior : CampaignBehaviorBase
    {
        ...
        {
            double chance = BASE_LEAK_CHANCE;
            ...
            chance -= ageInYears * 0.002; // -0.2% per year
            ...
            return MathF.Max(0f, MathF.Min(0.5f, (float)chance)); // Cap between 0% and 50%
        }
        ...
        private void GenerateLeak(Alliance alliance, Clan leakSource, float severity, string context)
        {
            ...
            alliance.AddHistoryEntry($"Information leaked by {leakSource?.Name.ToString() ?? "unknown source"} (Severity: {severity:F2})");
